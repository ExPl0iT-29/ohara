...
The system SHALL persist Content entities in a relational table with a schema matching the domain entity's fields, including a database-level enumeration for contentType and status.

#### Scenario: Content entity round-trips through persistence
- **WHEN** a Content entity is saved to the database and then retrieved by id
